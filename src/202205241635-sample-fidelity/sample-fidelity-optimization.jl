...
function run_optimization(layoutid, nrotorpoints; case="high-ti", tuning="sowfa-nrel", plotresults=false, verbose=true, wec=true, alpha=0, savehistory=false, optimize=true, outdir="./", layoutdir="../inputfiles/farms/startinglayouts/individual/", lspacing=3.0)
    ...
    params_calc = set_up_base_params(params, alpha=alpha, nrotorpoints=nrotorpoints)
...
function run_optimization_series(nruns, case, tuning, nrotorpoints; outdir="./", layoutgen="individual", wec=true, lspacing=3.0, firstrun=1, verbose=false, plotresults=false, savehistory=true)
    ...
    aepinitbasedata = []
    aepfinalbasedata = []
    aepinitcalcdata = [] # nrotorpoints rotor sample points
    aepfinalcalcdata = [] # 100 rotor sample points
    infodata = []
    outdata = []
    timedata = []
    fcalldata = []
...
function run_rotor_point_fidelity_study(;runsperbin=100)
    nrotorpoints=[1, 5, 10, 20, 40, 60, 80, 100]
    ...
    # end
    for npoints in nrotorpoints
        run_optimization_series(runsperbin, "high-ti", "sowfa-nrel", npoints; outdir="./high-ti/", layoutgen="angle-each-circle", wec=true, lspacing=5.0, verbose=false, plotresults=false, savehistory=true)
    end
end